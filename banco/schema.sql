...
CREATE TABLE IF NOT EXISTS cardapioitem (
  ...
  -- Se desejar, pode tornar o nome único:
  -- , UNIQUE (nome)
);
...
CREATE TABLE IF NOT EXISTS reserva (
  ...
  id_mesa      BIGINT NOT NULL REFERENCES mesa(id_mesa)     ON DELETE RESTRICT,

  CHECK (fim > inicio)
  -- Opcional: evita a mesma mesa no mesmo intervalo EXATO
...
CREATE TABLE IF NOT EXISTS pedido (
  ...
  -- Decisão de negócio: ao excluir o cliente, apagar seus pedidos
  id_cliente  BIGINT NOT NULL REFERENCES cliente(id_cliente) ON DELETE CASCADE
);

-- Pedido_Item (tabela de junção N:N entre Pedido e CardapioItem)
-- (padronização de nomes: usar id_item, igual à PK de cardapioitem)
CREATE TABLE IF NOT EXISTS pedido_item (
  id_pedido  BIGINT NOT NULL REFERENCES pedido(id_pedido)         ON DELETE CASCADE,
  id_item    BIGINT NOT NULL REFERENCES cardapioitem(id_item)     ON DELETE RESTRICT,
  quantidade INTEGER NOT NULL CHECK (quantidade > 0),

  PRIMARY KEY (id_pedido, id_item)
);

-- 4) Índices auxiliares para FKs
